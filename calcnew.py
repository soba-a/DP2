...
r1x, r1z, r2x, r2z = sp.symbols('r1x r1z r2x r2z')
R1 = sp.Matrix([r1x, 0, r1z]).T # no axial felt by shaft at bearing location
R2 = sp.Matrix([r2x, 0, r2z]).T # no axial felt by shaft at bearing location

# Known loads
Fp = sp.Matrix([-130.7, 0, 0]).T
Fb = sp.Matrix([42.8 * np.cos(np.radians(180 - 111.4)), 0, 42.8 * np.sin(np.radians(180-111.4))]).T
Fa_p = sp.Matrix([0, 10, 0]).T
Fa_p_r = sp.Matrix([0, -10, 0]).T
...
sumF = R1 + R2 + Fp + Fb + Fa_p + Fa_p_r # we know this equals 0
print("sumF: ", sumF)
...
MFa_at_r1 = sp.Matrix([0, 115 - bearing_width/2, -4*25.4]).T.cross(Fa_p_r)
...
# solve for unknowns
sol_loads = sp.solve(syseq, [r1x, r1z, r2x, r2z])
...
S_e_fp = calc_Se(Se_prime, ka=1, kb=0.9, kc=1, kd=1, ke=0.753, kf=1)
print("Se_fp:", S_e_fp, "MPa")
...
S_e_2 = calc_Se(Se_prime, ka=1, kb=calc_Kb(d_2p / 25.4), kc=1, kd=1, ke=0.753, kf=1) # with correct kb value
print("kb:", calc_Kb(d_2p/25.4))
print("Se:", S_e_2)
...
print("sig_a_p:", sig_a_p)
print("sig_m_p:", sig_m_p)
...
n = static_check_groove(F=Fa_p[1], M=Ma, Kt=Kt, T=Tm, d=d_2p, Kts=Kts, S_y=S_y)
...
# S_e & Marin Factors: see notes for reasoning. kb is different!
S_e_3 = calc_Se(Se_prime, ka=1, kb=calc_Kb(d_3p/25.4), kc=1, kd=1, ke=0.753, kf=1) # with correct kb value
print("kb:", calc_Kb(d_3p/25.4))
print("Se:", S_e_3)
...
print("Yield Check Von Mise:", n)
# check static yield check
n = static_check_groove(F=Fa_p[1], M=Ma, Kt=Kt, T=Tm, d=d_3p, Kts=Kts, S_y=S_y)
print("Static Yield Check: ", n)
...
L = (2 * F * n) / (w * S_y_key)
print("minimum key Length:", L)
L2 = 2 * F / h / S_y_key * n
print("mimimum key length2:", L2 / 25.4)
# key length = 1/2 "
l_key = 0.5 * 25.4
n_key = (S_y_key * w * l_key) / (2 * F)
print("n_key:", n_key)
n_key = (S_y_key * h/2 * l_key) / (F)
print("n_key:", n_key)
